...
export function queueBlockReason(ids, invitations, enabled, maximum = 15) {
  ...
  if (ids.length > maximum) return `Choose at most ${maximum} connection requests${maximum===15?' per batch':''}.`;
...
export function manualPersonStatus(contact, data) {
  ...
  if (items.some(i => i.status === 'failed')) return 'Needs reconciliation';
  if (items.some(i => i.status === 'prepared' && i.batch_status === 'running')) return 'Running';

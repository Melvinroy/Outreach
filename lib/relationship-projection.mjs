...
export function projectRelationships(snapshot, {manual=null, evidence=[], recommendations=[], activities=[], now=Date.now(), evidenceAvailable=true}={}) {
  ...
  const groups=Object.fromEntries(['all','discovery','outside','to_connect','execution','running','waiting','conversations','prepare','review','attention','do_not_contact','withdrawal','reply_needed','follow_up'].map(k=>[k,[]]));
  ...
  for(const c of contacts) {
    ...
    const outcomes=items.map(deliveryOutcome);
    ...
    if(!excluded && !['closed','snoozed'].includes(stage)) {
      ...
      if(queued) groups.execution.push(id);

      if(outcomes.includes('running')) groups.running.push(id);

      if(!queued && !reply && !follow && (outcomes.includes('sent') || c.connection_status==='request_sent' || (!manual && stage==='waiting'))) groups.waiting.push(id);

      

      if(!queued && (reply || follow)) groups[tasks.some(t=>t.status==='needs_review' && t.draft_message) || (!manual && stage==='review') ? 'review':'prepare'].push(id);

    } else if(excluded) groups.do_not_contact.push(id);

    if(connected.has(id) && !excluded) groups.conversations.push(id);
    groups.all.push(id);

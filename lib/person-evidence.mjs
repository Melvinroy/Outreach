...
export function conversationEvidence(messages = [], invitations = [], replies = [], events = []) {
  const sent = (rows,kind) => rows.filter(s=>s.status==='completed' && s.completed_at && s.confirmation_signal && s.message_snapshot?.trim()).map(s=>({
    ...
    occurred_at:s.completed_at,source:kind==='invitation'?'Confirmed invitation':'Confirmed message',verified:false,
  }));
...
export const invitationEvidenceFields = 'id,contact_id,status,message_snapshot,confirmation_signal,completed_at,failure_reason,skip_reason,preflight_evidence';

...
test("fresh installation defines every dashboard core table before later phases", () => {
  ...
  assert.doesNotMatch(
    migration,
    /@gmail\.com|appgprj_|sb_publishable_[A-Za-z0-9_-]{8,}/,
  );
});

test("first-owner claim cannot replace an existing owner", () => {
  const migration = read(
    "supabase/migrations/012_phase_six_productization.sql",
  );
  assert.match(
    migration,
...
test("public landing precedes sign-in while private workspace configuration remains available", () => {
  const setup = read("components/outreach-setup.tsx");
  const page = read("app/page.tsx");
  const deployment = read("lib/outreach-deployment.ts");
  assert.match(page, /if \(!session \|\| !supabase\) return/);
  assert.match(page, /get\("view"\) === "sign-in"/);
  assert.match(page, /<LandingPage\/>/);
  assert.match(
    page,
    /runtimeEnv\.VITE_SUPABASE_URL\s*\?\?\s*deploymentCloudConfig\?\.url\s*\?\?\s*storedCloudConfig\?\.url/,
  );
  assert.match(page, /<SignIn client=\{supabase\}\/>/);
  assert.match(page, /simple-sign-in/);
  assert.match(read("components/application-shell.tsx"), /Explore sample workspace/);
  assert.match(read("components/control-center.tsx"), /<WorkspaceConnectionDialog\/>/);
  ...
  assert.match(
    deployment,
    /OUTREACH_SUPABASE_PUBLISHABLE_KEY = "sb_publishable_/,
  );
  assert.doesNotMatch(
    deployment,
  ...
  assert.match(
    styles,
    /\.sign-in-card \.gate-icon[^}]*width: 36px[^}]*height: 36px/,
  );
  assert.match(
    styles,
    /demo-outreach-table \.actions-cell \{ display: table-cell/,
  );
  assert.match(
    styles,
    /demo-account-menu \.workspace-connect[^}]*font-size: 0!important/,
  );
  ...
  assert.match(
    styles,
    /outreach-table td[^}]*height: 50px[^}]*font-size: 12px/,
  );
  assert.match(
    styles,
    /queue-card,\.demo-product \.queue-card[^}]*height: auto[^}]*border-radius: 8px/,
  );
  assert.match(
    styles,
    /Shared typography hierarchy across every dashboard tab/,
  );
  assert.match(
    styles,
    /Unified institutional surfaces across every dashboard view/,
  );
  assert.match(page, /PRIORITY QUEUE/);
  assert.match(
    page,
    /demoScope === "unreached"[\s\S]*?\? "Unreached people"[\s\S]*?: "Outreach people"/,
  );
  assert.match(page, /demo-funnel-head/);
  assert.match(
    page,
    />Stage<\/span>\s*<span>Progress<\/span>\s*<strong>Total<\/strong>/,
  );
  assert.match(styles, /demo-product \.demo-funnel > \.demo-funnel-head/);
  assert.match(
    styles,
    /demo-product \.demo-panel-list > div[^}]*border: 0[^}]*border-bottom: 1px solid #e3e9ee/,
  );
  assert.match(
    styles,
    /demo-product \.demo-funnel i b[^}]*background: #1769aa/,
  );
  assert.doesNotMatch(
    styles,
    /demo-product \.demo-funnel i b \{ background: linear-gradient/,
  );
  assert.match(
    styles,
    /demo-product \.demo-workflow-grid > div[^}]*border-radius: 0/,
  );
  assert.match(
    styles,
    /person-cell strong,\.demo-product \.person-cell strong[^}]*color: #25364a[^}]*font-weight: 600/,
  );
  assert.match(
    styles,
    /person-cell strong,\.followup-person strong,\.conversation-task-person strong,\.demo-panel-list strong,\.demo-workflow-grid strong,\.workflow-row strong[^}]*font-size: 12px/,
  );
  assert.match(
    styles,
    /demo-funnel > div,\.demo-funnel strong[^}]*font-size: 12px/,
  );
  ...
  assert.match(
    setup,
    /Secret keys, database passwords and LinkedIn credentials are rejected/,
  );
});

test("automation templates are placeholders and never send automatically", () => {
  for (const file of [
    "automations/linkedin-acceptance-sync.md",
    "automations/linkedin-message-sync.md",
  ]) {
...
test("authenticated browser users cannot truncate workflow tables", () => {
  const migration = read(
    "supabase/migrations/013_revoke_broad_authenticated_privileges.sql",
  );
  assert.match(migration, /revoke truncate, references, trigger/i);
  for (const table of [
    "outreach_assist_batches",
    "outreach_assist_sessions",
    "outreach_user_settings",
  ]) {
...
test("release CI enforces the complete verification gate", () => {
  ...
  assert.doesNotMatch(
    workflow,
    /git push\s+--force|one_time_history_finalize|commit-tree/,
  );
});
